...
import { X, Copy, Check, Search, MessageCircle, Facebook, Send, Loader2, BadgeCheck } from "lucide-react";
...
export default function ShareModal({ isOpen, onClose, postUrl, postTitle, postId, postImage }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={handleClose}>
      <div
        className="flex max-h-[85vh] w-full max-w-md flex-col rounded-2xl bg-base-100 shadow-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="relative flex shrink-0 items-center justify-center border-b border-base-300 p-4">
          <h3 className="text-base font-semibold text-base-content">Share</h3>
          <button
            type="button"
            onClick={handleClose}
            className="absolute left-4 rounded-full p-1 text-base-content/50 hover:bg-base-200 hover:text-base-content/70 transition-colors"
          >
            <X className="size-5" />
          </button>
        </div>
        ...
        <div className="shrink-0 p-4 pb-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-full border border-base-300 bg-base-200 py-2 pl-9 pr-3 text-sm text-base-content outline-none focus:border-primary/30"
            />
          </div>
        </div>
        ...
        <div className="max-h-[248px] shrink-0 overflow-y-auto p-4 pt-2">
          {isLoadingFriends ? (
            <p className="py-6 text-center text-sm text-base-content/60">Loading your friends...</p>
                ...
                return (
                  ...
                  >
                    <div className="relative">
                      ...
                      {isSelected && (
                        <span className="absolute -bottom-0.5 -right-0.5 grid size-5 place-items-center rounded-full border-2 border-white bg-primary">
        ...
        {selectedFriendIds.size > 0 && (
          <div className="shrink-0 px-4 pb-2">
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-2.5 text-sm font-semibold text-white hover:bg-primary transition-colors disabled:opacity-60"
            >
        ...
        <div className="shrink-0 border-t border-base-300 p-4">
          <div className="grid grid-cols-3 gap-3">
            <button
              ...
              className="flex flex-col items-center gap-1.5 rounded-xl p-2 text-base-content/70 hover:bg-base-200 transition-colors"
            >
              <span className="grid size-11 place-items-center rounded-full bg-base-200">
            ...
            >
              <span className="grid size-11 place-items-center rounded-full bg-base-200">
                <Facebook className="size-5 text-info" />
              </span>
