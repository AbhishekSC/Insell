  ...
  return (
    <section className="rounded-2xl border border-base-300 bg-base-100 p-4 shadow-[0_2px_10px_rgba(15,23,42,0.035)] sm:p-5">
      ...
      <div className="mt-3 divide-y divide-base-300">
        {users.length ? users.slice(0, 4).map((user) => <div key={user._id} className="flex items-center gap-3 py-3"><UserAvatar src={user.profilePic} name={user.fullName || "User"} sizeClass="size-10" userId={user._id} /><div className="min-w-0 flex-1"><span className="flex min-w-0 items-center gap-1"><Link to={`/users/${user._id}`} className="truncate text-sm font-bold text-base-content hover:text-primary">{user.fullName || "Unknown user"}</Link>{user?.isOwnerVerified && <ShieldCheck className="size-3.5 shrink-0 text-primary" aria-label="Verified Owner" />}</span><p className="mt-0.5 truncate text-xs text-base-content/60">{userRole(user)}</p><p className="mt-0.5 inline-flex items-center gap-1 text-[11px] text-base-content/50"><MapPin className="size-3" />{userLocation(user)}</p></div><button type="button" onClick={() => onConnect(user._id)} disabled={busy && entityId(user) === entityId(pendingUserId)} className="btn btn-sm h-8 rounded-lg border border-primary/30 bg-base-100 px-2.5 text-xs text-primary shadow-none hover:border-primary/30 hover:bg-primary/10"><UserPlus className="size-3.5" />Connect</button></div>) : <p className="py-6 text-center text-sm text-base-content/60">No matching people found.</p>}
      </div>
    </section>
  );
}
