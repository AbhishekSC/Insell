...
export default function CallPage() {
  ...
  // Real online status — same mechanism as the Messages list. Without this,
  // every friend who simply isn't on a call showed as green regardless of
  // whether they were actually online at all.
      ...
      <div className="call-lobby-grid xl:h-full xl:min-h-0">
          ...
          <div className="flex flex-1 flex-col p-4 sm:p-6">
            ...
            {!videoClient ? (
              ...
              <div className="call-preview-card flex flex-1 flex-col justify-center p-6 sm:p-10">
                {selectedFriends.length === 1 ? (
                  <div className="mx-auto w-full max-w-sm text-center"><UserAvatar src={selectedFriends[0].profilePic} name={selectedFriends[0].fullName} sizeClass="size-20" className="mx-auto ring-4 ring-primary/10" /><p className="mt-4 flex items-center justify-center gap-1.5 text-lg font-bold"><span>Call {selectedFriends[0].fullName}</span>{selectedFriends[0]?.isOwnerVerified && <BadgeCheck className="size-4 shrink-0 text-primary" aria-label="Verified Owner" />}</p><p className="mt-1 text-sm text-base-content/60">{selectedFriends[0].travelStyle || selectedFriends[0].learningLanguage || "Travel partner"} · {selectedFriends[0].homeBase || selectedFriends[0].location || "Planning together"}</p><button type="button" className="btn btn-primary mt-6 w-full rounded-xl" onClick={startCall} disabled={videoBusy || !serviceReady}>{videoBusy ? <Loader2 className="size-5 animate-spin" /> : <Video className="size-5" />} Start video call</button><p className="mt-3 text-xs text-base-content/50">A private room will be created for both of you.</p></div>
                ) : selectedFriends.length > 1 ? (
        ...
        <aside className="call-sidebar shell-panel xl:min-h-0 xl:overflow-y-auto">
          <div className="p-4 sm:p-5">
            ...
            {isLoading ? <div className="mt-4 h-28 animate-pulse rounded-2xl bg-base-200" /> : friends.length === 0 ? <div className="empty-box mt-4 text-sm">No friends available yet. Build your network from Requests.</div> : <div className="call-friend-list">{friends.map((friend) => { const isSelected = selectedFriendIds.includes(friend._id); const inCall = presenceByFriendId[friend._id]?.status === "busy"; const isOnline = onlineFriendIds.has(friend._id); return <button key={friend._id} type="button" className={`call-friend-row ${isSelected ? "call-friend-row--selected" : ""}`} onClick={() => toggleFriendSelection(friend._id)}><span className="relative shrink-0"><UserAvatar src={friend.profilePic} name={friend.fullName} sizeClass="size-10" />{inCall ? <span className="absolute -bottom-0.5 -right-0.5 size-3 rounded-full border-2 border-base-100 bg-error" title="In a call" /> : isOnline ? <span className="absolute -bottom-0.5 -right-0.5 size-3 rounded-full border-2 border-base-100 bg-success" title="Online" /> : null}</span><span className="min-w-0 flex-1 text-left"><span className="flex items-center gap-1 truncate text-sm font-semibold"><span className="truncate">{friend.fullName}</span>{friend?.isOwnerVerified && <BadgeCheck className="size-3.5 shrink-0 text-primary" aria-label="Verified Owner" />}</span><span className={`block truncate text-xs ${inCall ? "font-medium text-error" : "text-base-content/55"}`}>{inCall ? "In a call" : friend.travelStyle || friend.learningLanguage || "Travel partner"}</span></span>{isSelected ? <Check className="size-4 text-primary" /> : <ChevronRight className="size-4 text-base-content/35" />}</button>; })}</div>}

            <div className="call-privacy-note"><BadgeCheck className="size-4" /><span>Private rooms are only visible to invited members.</span></div>
