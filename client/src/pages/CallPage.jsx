          ...
          <div className="flex flex-1 flex-col p-4 sm:p-6">
            ...
            {!videoClient ? (
              ...
              <div className="call-preview-card flex flex-1 flex-col justify-center p-6 sm:p-10">
                {selectedFriends.length === 1 ? (
                  <div className="mx-auto w-full max-w-sm text-center"><UserAvatar src={selectedFriends[0].profilePic} name={selectedFriends[0].fullName} sizeClass="size-20" className="mx-auto ring-4 ring-primary/10" /><p className="mt-4 flex items-center justify-center gap-1.5 text-lg font-bold"><span>Call {selectedFriends[0].fullName}</span>{selectedFriends[0]?.isOwnerVerified && <ShieldCheck className="size-4 shrink-0 text-primary" aria-label="Verified Owner" />}</p><p className="mt-1 text-sm text-base-content/60">{selectedFriends[0].travelStyle || selectedFriends[0].learningLanguage || "Travel partner"} · {selectedFriends[0].homeBase || selectedFriends[0].location || "Planning together"}</p><button type="button" className="btn btn-primary mt-6 w-full rounded-xl" onClick={startCall} disabled={videoBusy || !serviceReady}>{videoBusy ? <Loader2 className="size-5 animate-spin" /> : <Video className="size-5" />} Start video call</button><p className="mt-3 text-xs text-base-content/50">A private room will be created for both of you.</p></div>
                ) : selectedFriends.length > 1 ? (
